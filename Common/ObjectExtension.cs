...
namespace Common
{
    ...
    public static class ObjectExtension
    {
        ...
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="exceptPropertyName"></param>
        /// <returns></returns>
        public static bool SimplePropertyEquals<T>(this T x, T y, params string[] exceptPropertyName)
        {
            if (x == null || y == null)
            {
                throw new ArgumentNullException("x,y");
            }
            foreach (PropertyInfo pif in (from ppt in typeof(T).GetProperties() where exceptPropertyName.Contains(ppt.Name, (a, b) => { return string.Equals(a, b, StringComparison.InvariantCultureIgnoreCase); }) == false select ppt))
            {
                object objXValue = pif.GetValue(x, null);
                object objYValue = pif.GetValue(y, null);
                if (objXValue == null)
                {
                    if (objYValue == null)
                    {
                ...
                if (objXValue.Equals(objYValue) == false)
                {
                    return false;
                }
            }
        ...
        /// <summary>
        /// 从一组对象的属性中提取对应值合并到原始对象（value参数对应的）中。如：TopicDetail包含了Topic对象的信息和User对象的信息，且属性名称唯一（不同对象间引用的同名的属性不会有二义性），则可以通过下述方式生成TopicDetail对象：TopicDetail tpd=new TopicDetail();tpd.MergeProperty(topic1,user1);其效果等同于：tpd.UserName=user1.UserName;tpd.TopicId=topic1.TopicId;tpd.PostTime=topic1.PostTime;...
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="value"></param>
        /// <param name="sources"></param>
        /// <returns></returns>
        public static T MergeProperty<T>(this T value, params object[] sources)
        {
            sources.Foreach(o => { value.MergeProperty(o); });
            return value;
        }
        ...
        /// <returns></returns>
        public static IEnumerable<KeyValuePair<string, object>> PropertyToKeyValueList(this object value, BindingFlags flags = BindingFlags.Public | BindingFlags.Instance, params string[] excludedProperties)
        {
            if (value == null)
            {
                yield break;
            ...
            foreach (PropertyInfo pif in typ.GetProperties(flags))
            {
                if (excludedProperties.Contains(pif.Name, (x, y) => { return string.Equals(x, y, StringComparison.InvariantCultureIgnoreCase); }))
                {
                    continue;
                }
                yield return new KeyValuePair<string, object>(pif.Name, pif.GetValue(value, null));
            }
        }
        ...
        /// <param name="excludedProperties"></param>
        /// <returns></returns>
        public static IEnumerable<KeyValuePair<string, object>> PropertyToKeyValueList(this object value, params string[] excludedProperties)
        {
            return value.PropertyToKeyValueList(BindingFlags.Instance | BindingFlags.Public, excludedProperties);
        }
